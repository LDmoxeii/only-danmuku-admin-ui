<template>
  <div>
    <el-table ref="dataTable" :data="dataSource.list || []" :height="tableHeight" :stripe="options.stripe" :border="options.border" header-row-class-name="table-header-row" highlight-current-row @row-click="handleRowClick" @selection-change="handleSelectionChange">
      <el-table-column v-if="options.selectType && options.selectType === 'checkbox'" type="selection" :selectable="selectedHandler" width="50" align="center"/>
      ...
      <template v-for="(column, index) in columns" :key="index">
        <template v-if="(column as any).scopedSlots">
          <el-table-column :prop="(column as any).prop" :label="(column as any).label" :align="(column as any).align || 'left'" :width="(column as any).width">
        ...
        <template v-else>
          <el-table-column :prop="(column as any).prop" :label="(column as any).label" :align="(column as any).align || 'left'" :width="(column as any).width" :fixed="(column as any).fixed"/>
        </template>
      </template>
    </el-table>
    <div class="pagination" v-if="showPagination">
      <el-pagination v-if="dataSource.totalCount" background :total="dataSource.totalCount" :page-sizes="[15, 30, 50, 100]" :page-size="dataSource.pageSize" :current-page="dataSource.pageNum" layout="total, sizes, prev, pager, next, jumper" @size-change="handlePageSizeChange" @current-change="handlepageNumChange" style="text-align: right"/>
    </div>
  </div>
</template>
...
const extHeight = computed(() => props.extHeight)
